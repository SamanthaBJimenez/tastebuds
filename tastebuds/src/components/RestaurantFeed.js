...
const RestaurantFeed = ({ restaurants }) => {
  const restaurantList = restaurants.map((restaurant, i) => {
    return (
      <Link key={i} to={`/restaurant/${restaurant.restaurant_id}`}>
        <li className="restaurant_li">
          <button
  ...
  return (
    <div>
      {restaurantList.length ? (
        <ul className="restaurant_ul">{restaurantList}</ul>
      ) : (
        <div className="feedBlurbDiv">
          <h1 className="feed_blurbOne">Where Are We Going To Eat?</h1>
          <h3 className="feed_blurbTwo">
            Type your zip code in the search bar to find restaurants near you.
          </h3>
        </div>
      )}
    </div>
